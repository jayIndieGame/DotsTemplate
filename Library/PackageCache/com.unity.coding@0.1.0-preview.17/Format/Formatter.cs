    ...
    static class Formatter
    {
        ...
        public static IDictionary<string, long> Process(IEnumerable<NPath> pathsToFormat, FormatContext formatContext, Func<FormatItem, bool> cancellableBeforeFormatCallback = null)
        {
            ...
            foreach (var path in pathsToFormat)
            {
                ...
                {
                    formatContext.Logger.Error($"Path is a directory: {path}");
                    continue;
                }

                if (!path.FileExists())
                {
                    formatContext.Logger.Error($"File doesn't exist: {path}");
                    continue;
                }
            ...
            if (formatItems.Any())
            {
                ...
                sw.Stop();
                string operation = (formatContext.Options & FormatOptions.DryRun) == 0 ? "Formatted" : "Validated";
        ...
        static bool OverwriteFileIfChanged(FormatItem formatItem, IEnumerable<string> lines, EndOfLine eol)
        {
            ...
            if (!match && (formatItem.Context.Options & FormatOptions.DryRun) == 0)
            {
